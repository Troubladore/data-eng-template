...
# Change to project root (inside devcontainer, this is /workspaces/{{cookiecutter.project_slug}})
# But if we're in the project directory already, stay here
if [ -d "/workspaces/{{cookiecutter.project_slug}}" ]; then
    cd "/workspaces/{{cookiecutter.project_slug}}"
