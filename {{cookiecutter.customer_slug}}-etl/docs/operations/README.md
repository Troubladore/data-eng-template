...
docker run -e ENVIRONMENT=prod {{cookiecutter.project_slug}}:latest \
  python scripts/run_pipeline.py environment=prod
```
...
python -c "from {{cookiecutter.project_slug.replace('-', '_')}}.config import get_settings; print('OK')"
...
python -c "from {{cookiecutter.project_slug.replace('-', '_')}}.config import get_settings; get_settings()"

# Check specific environment
python -c "from {{cookiecutter.project_slug.replace('-', '_')}}.config import get_settings; get_settings(environment='prod')"
```
...
python -c "from {{cookiecutter.project_slug.replace('-', '_')}}.config import get_settings; s=get_settings(); print(s.database.connection_url)"
```
