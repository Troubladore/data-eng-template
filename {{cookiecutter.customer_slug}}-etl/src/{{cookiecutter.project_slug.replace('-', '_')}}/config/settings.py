...
class ProjectSettings(BaseModel):
    ...
    # Version settings (populated from cookiecutter variables)
    python_version: str = Field("3.12", description="Python version requirement")
    airflow_version: str = Field("2.9.3", description="Airflow version")
    postgres_version: str = Field("16", description="PostgreSQL version")
...
class DatabaseSettings(BaseModel):
    ...
    db_schema: Optional[str] = Field(None, description="Default schema", alias="schema")
...
class OrchestrationSettings(BaseModel):
    ...
    # Security settings (generated by post-generation hook)
    fernet_key: Optional[str] = Field(None, description="Airflow Fernet encryption key")
    admin_username: str = Field("admin", description="Default admin username")
...
class DbtProjectSettings(BaseModel):
    """dbt project configuration."""
    name: str = Field(..., description="Project name")
    version: str = Field("1.0.0", description="Project version") 
    profile: str = Field(..., description="Profile name")
    model_paths: List[str] = Field(["models"], description="Model paths")
    target_path: str = Field("target", description="Target path")
    log_path: str = Field("logs", description="Log path")

class DbtProfileSettings(BaseModel):
...
class TransformationSettings(BaseModel):
    ...
    # Project settings
    project: DbtProjectSettings
    profile: DbtProfileSettings
    
    # Execution settings (optional overrides)
    threads: Optional[int] = Field(None, description="Number of threads")
    full_refresh: Optional[bool] = Field(None, description="Full refresh mode")
    
    # Development features (optional overrides)
    partial_parse: Optional[bool] = Field(None, description="Enable partial parsing")
    use_colors: Optional[bool] = Field(None, description="Colored output")
    warn_error: Optional[bool] = Field(None, description="Treat warnings as errors")
...
class Settings(BaseModel):
    ...
    hydra_cfg: Optional[DictConfig] = Field(None, exclude=True)
    
    class Config:
        """Pydantic configuration."""
        extra = "allow"  # Allow additional fields from Hydra config
        arbitrary_types_allowed = True  # Allow DictConfig and other complex types
    ...
    def validate_settings(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "database" in values and "project" in values:
            database_data = values["database"]
            project_data = values["project"]
            
            # Handle both dict and object cases
            if isinstance(database_data, dict):
                if database_data.get("db_schema") is None:
                    if isinstance(project_data, dict):
                        database_data["db_schema"] = project_data.get("slug")
                    else:
                        database_data["db_schema"] = getattr(project_data, "slug", None)
    ...
    def is_development(self) -> bool:
        ...
        return self.runtime.debug_mode or self.transformations.profile.target == "dev"
    
    @property
    def is_production(self) -> bool:
        """Check if running in production mode."""
        return self.transformations.profile.target == "prod" and not self.runtime.debug_mode
