...
build_airflow() {
    ...
    if [[ "${NO_CACHE:-}" == "true" ]]; then
        docker build -f Dockerfile.airflow -t "{{cookiecutter.project_slug}}-airflow" .
    else
        python3 "$FINGERPRINT_SCRIPT" \
            "$PROJECT_ROOT" \
            "Dockerfile.airflow" \
            "{{cookiecutter.project_slug}}-airflow" \
            "airflow" \
...
build_data_processing() {
    ...
    if [[ "${NO_CACHE:-}" == "true" ]]; then
        docker build -f Dockerfile.data-processing -t "{{cookiecutter.project_slug}}-data-processing" .
    else
        python3 "$FINGERPRINT_SCRIPT" \
            "$PROJECT_ROOT" \
            "Dockerfile.data-processing" \
            "{{cookiecutter.project_slug}}-data-processing" \
            "data-processing" \
...
validate_environments() {
    ...
    docker run --rm "{{cookiecutter.project_slug}}-airflow" python -c "
...
"
    ...
    docker run --rm "{{cookiecutter.project_slug}}-data-processing" python -c "
import sqlalchemy as sa
import pandas as pd
print(f'✅ SQLAlchemy: {sa.__version__}')
